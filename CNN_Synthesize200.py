...
classes = {'seastate_ss200_2','images_ss200_2'}
...
def discriminator(data, isTrain=True, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        x = tf.reshape(data, shape=[-1,200,200,1])
        x = tf.layers.conv2d(x, kernel_size=5, filters=16, strides=2, padding='same', activation=activation)
...
NN_input = tf.placeholder(dtype=tf.float32, shape=[None, 200, 200], name='NN_input')
...
with tf.control_dependencies(update_ops):
    optimizer_d = tf.train.RMSPropOptimizer(learning_rate=0.00005).minimize(loss, var_list=vars_d)
